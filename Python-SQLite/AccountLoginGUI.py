...
class MyBackground(model.Background):
   ...
   #The login button
    def on_cmdLogin_mouseClick(self, event):
        ...
        currentUser = self.components.txtCurrentUser.text
        currentPass = self.components.txtAuthKey.text
        ...
        if row is None:
            ...
            if fetchedHash == currentHash:
                dialog.alertDialog(self, "Login Success", 'Session Login')
            else:
                dialog.alertDialog(self, "Login Fail", 'Session Login')

        
    #The Create Account button
    def on_cmdNewAccount_mouseClick(self, event):

        #Get username and password
        currentUser = self.components.txtCurrentUser.text

        #Hash the user's password
        currentPassword = hashlib.sha256(self.components.txtAuthKey.text).hexdigest()

        #Write entry to database table and commit
        c.execute("insert into Hashes values (?, ?)", (currentUser, currentPassword))
        conn.commit()

        dialog.alertDialog(self, "Account Created", 'Session Login')
